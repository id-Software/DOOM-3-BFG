...
float3 getPosition( int2 ssP, Texture2D cszBuffer )
{
	float3 P;
...
void getPositions( int2 ssP, Texture2D cszBuffer, out float3 P0, out float3 P1 )
{
	float2 Zs = texelFetch( cszBuffer, ssP, 0 ).rg;

	// Offset to pixel center
	P0 = reconstructCSPosition( float2( ssP ) + _float2( 0.5 ), Zs.x );
	P1 = reconstructCSPosition( float2( ssP ) + _float2( 0.5 ), Zs.y );
}


void computeMipInfo( float ssR, int2 ssP, Texture2D cszBuffer, inout int mipLevel, inout int2 mipP )
{
#if USE_MIPMAPS
	// Derivation:
...
float3 getOffsetPosition( int2 ssC, float2 unitOffset, float ssR, Texture2D cszBuffer, float invCszBufferScale )
{
	float3 P;
#if USE_DEPTH_PEEL
	int2 ssP = clamp( _int2( ssR * unitOffset ) + ssC, _int2( 0 ), _int2( g3d_sz2D_colorBuffer.xy - 1 ) );
	...
	// Offset to pixel center
	P = reconstructCSPosition( ( float2( ssP ) + _float2( 0.5 ) ) * invCszBufferScale, P.z );
#endif
...
void getOffsetPositions( int2 ssC, float2 unitOffset, float ssR, Texture2D cszBuffer, out float3 P0, out float3 P1 )
{
#if USE_DEPTH_PEEL
	int2 ssP = clamp( _int2( ssR * unitOffset ) + ssC, _int2( 0 ), _int2( g3d_sz2D_colorBuffer.xy - 1 ) );
  ...
  inout float3     n_tap )
{
	...
	// Offset to pixel center
	Q = reconstructCSPosition( ( float2( ssP ) + _float2( 0.5 ) ), z );
}
  ...
  out float3     n_tap1 )
{
	...
	// Offset to pixel center
	//Q0 = reconstructCSPosition( ( float2( ssP ) + _float2( 0.5 ) ), Zs.x, projInfo );
	//Q1 = reconstructCSPosition( ( float2( ssP ) + _float2( 0.5 ) ), Zs.y, projInfo );
}
...
( in int2             ssC,
  ...
  in Texture2D		  cszBuffer,
  in Texture2D		  nBuffer,
  in Texture2D		  bounceBuffer,
...
void main( PS_IN fragment, out PS_OUT result )
{
	...
	float3 C_peeled = _float3( 0 );
	float3 n_C_peeled = _float3( 0 );
#endif
