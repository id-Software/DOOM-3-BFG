...

		{
			...
			for( int n = 0; n < IM_ARRAYSIZE( names ); n++ )
			{
				...
				if( ImGui::BeginDragDropSource( ImGuiDragDropFlags_None ) )
				{
					...
					if( mode == Mode_Copy )
					{
						ImGui::Text( "Copy %s", names[n] );    // Display preview (could be anything, e.g. when dragging an image we could decide to display the filename and a small preview of the image, etc.)
					}
					if( mode == Mode_Move )
					{
...
static void ShowDemoWindowLayout()
{
	...
	if( ImGui::TreeNode( "Text Baseline Alignment" ) )
	{
		...
		if( ImGui::TreeNode( "Node##1" ) )
		{
			for( int i = 0; i < 6; i++ )
			{
				ImGui::BulletText( "Item %d..", i );    // Dummy tree data
			}
			ImGui::TreePop();
		}
